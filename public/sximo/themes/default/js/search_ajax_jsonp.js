	...
	$(document).ready(function() {
      	var url = window.location.href;
		   		...
		   		$.getJSON(urlSearchAjaxGetTransportTypes+"?callback=?",{ holidayType: $('input[name="searchHolidayType"]:checked').val() },function(data){
		   			...
		   			for(var i = 0 ; i < transportTypes.length;i++){
		   				transportTypeField.append($("<option></option>").attr("value",transportTypes[i].id).text(transportTypes[i].name));
							...
							transportTypeField.parent().parent().parent().parent().parent().waitMe('hide');
					});
	   		} else {
	   			$('.input-hotel').show();
		   			...
		   			var countries = data;
						for(var i = 0 ; i < countries.length;i++){
		   				countryDestinationField.append($("<option></option>").attr("value",countries[i].id).text(countries[i].name));
							...
							countryDestinationField.parent().parent().parent().parent().parent().waitMe('hide');;
					});
	   		}
	   	});

	   	transportTypeField.change(function(){
					if(transportTypeField.hasClass('sbar')){
						elementSpin = transportTypeField.parent().parent().parent().parent();
					}else{
						elementSpin = transportTypeField.parent().parent().parent().parent().parent();
					}
					elementSpin.waitMe({
						effect: 'ios',
						text: '',
	   		...
	   		if(transportTypeField.val() != 0){
				...
				if (transportTypeField.val() != 3) {
					if(departurePointField.is(":visible") == false){departurePointField.show();}
					...
					//console.log(departurePointField.val())
					$.getJSON(urlSearchAjaxGetDeparturePoints+"?callback=?",{ holidayType: $('input[name="searchHolidayType"]:checked').val(), transportType: transportTypeField.val() },function(data){
						var countries = data;
						for(var i = 0 ; i < countries.length;i++){
							countryDestinationField.append($("<option></option>").attr("value",countries[i]['id']).text(countries[i]['name']));
	   	...
	   	departurePointField.change(function(){
				...
				if(departurePointField.hasClass('sbar')){
					elementSpin = departurePointField.parent().parent().parent().parent();
				}else{
					elementSpin = departurePointField.parent().parent().parent().parent().parent();
				}
				elementSpin.waitMe({
					effect: 'ios',
					text: '',
	   	...
	   	countryDestinationField.change(function(){
				if(countryDestinationField.hasClass('sbar')){
					elementSpin = countryDestinationField.parent().parent().parent().parent();
				}else{
					elementSpin = countryDestinationField.parent().parent().parent().parent().parent();
				}
				elementSpin.waitMe({
					effect: 'ios',
					text: '',
	   	...
	   	cityDestinationField.change(function(){
				if(cityDestinationField.hasClass('sbar')){
					elementSpin = cityDestinationField.parent().parent().parent().parent();
				}else{
					elementSpin = cityDestinationField.parent().parent().parent().parent().parent();
				}
				elementSpin.waitMe({
					effect: 'ios',
					text: '',
	   	...
	   	departureDateField.change(function(){
				if(departureDateField.hasClass('sbar')){
					elementSpin = departureDateField.parent().parent().parent().parent();
				}else{
					elementSpin = departureDateField.parent().parent().parent().parent().parent();
				}
				elementSpin.waitMe({
					effect: 'ios',
					text: '',
	   		...
	   		if(departureDateField.val() != 0){
		   		$.getJSON(urlSearchAjaxGetDurations+"?callback=?",{ holidayType: $('input[name="searchHolidayType"]:checked').val(), transportType: transportTypeField.val() , city: cityDestinationField.val(), departureDate: departureDateField.val(),  departurePoint: departurePointField.val() },function(data){
		   			...
		   			for(var i = 0 ; i < durations.length;i++){
		   				durationField.append($("<option></option>").attr("value",durations[i]).text(durations[i]+" zile"));
		   			}
		   		}).done(function(){
							elementSpin.waitMe('hide');
					});
	   	...
	   	searchButton.click(function(){
	   		...
	   		if(validateErrors(holidayTypeVal,transportTypeVal,countryDestinationVal,cityDestinationVal,departureDateVal,durationVal,departureDateHotelVal,arrivalDateHotelVal,departurePointVal)){
	            vex.open({
				  content: '<canter><img style="width: 99%;" src="/images/logo.jpg" /></center><center><img style="width: 25%;position: relative;top: -10px;" src="/images/loader.gif" /></center>',
