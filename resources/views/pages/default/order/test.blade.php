    ...
    $('#create-order').ajaxForm(function(response){
        ...
        if(response.status == "ERROR"){
            ...
            if($('#paytype').val() == 'Romcard'){
              ...
              $.get(link+'&packageid='+<?php echo App\Models\Travel\Eloquent\PackageInfoEloquent::find($bookingPackageSearch->id_package)->idx; ?>+'&paytype='+$('#paytype').val()).done(function(data){
  ...
  <div class="nicdark_container nicdark_vc nicdark_clearfix">
      <div class="ba-order-page">
        ...
        <div class="currency" style="display:none;">
          ...
          <p class="input_row"><input type="radio" name="payment-type" id="payment-type1" value="EUR"> <strong>EUR.</strong>  Platesti {{$bookingPackageSearch->price * 0.3}} EUR</p>
          <p class="input_row"><input type="radio" name="payment-type" id="payment-type2" value="RON"> <strong>RON.</strong>  Platesti {{number_format($bookingPackageSearch->price * \DB::table('curseuro')->where('id','=',1)->first()->curseuro * 0.3,2)}} RON</p>
        </div>
...
<?php
    ...
    $fields = array(
    	'AMOUNT'=>$bookingPackageSearch->price,
    	'CURRENCY'=>'EUR',
    	'ORDER'=>$bookingPackageSearch->id,
    ...
    $fields2 = array(
    	'AMOUNT'=>number_format($bookingPackageSearch->price * \DB::table('curseuro')->where('id','=',1)->first()->curseuro,2),
    	'CURRENCY'=>'RON',
    	'ORDER'=>$bookingPackageSearch->id,
    	'DESC'=>substr($package->name,0,50),
    	'MERCH_NAME'=>'HELLO HOLIDAYS SRL',
      ...
      <div class="ba-order-page">
        <div class="ba-order-details">
          ...
          <div class="ba-order-details-content">
            <div class="pay_half">
	            <span class="ba-order-details-hotel-name">{{{$package->name}}}</span>

	            <span class="ba-order-details-hotel-location"></span>
	            <br/>
