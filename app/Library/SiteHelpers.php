...
/*	public static function showUploadedFile($file,$path , $width = 50) {
		...
		//	echo $files ;
			...
			if($info['extension'] == "jpg" || $info['extension'] == "jpeg" ||  $info['extension'] == "png" || $info['extension'] == "gif" || $info['extension'] == "JPG")
			{
				...
				<img src="'.asset( $path_file . $file ).'" border="0" width="'. $width .'" class="img-square" /></a></p>';
			} else {
				$path_file = str_replace("./","",$path);
			...
			return "<img src='".asset('/uploads/images/no-image.png')."' border='0' width='".$width."' class='img-square' /></a>";
	...
	public static function showUploadedFile($file,$path , $width = 50) {
			...
			if(file_exists($files ) && $file !="") {
				...
				$ext = explode('.',$file);
				if($ext[1] == "jpg" || $ext[1] == "jpeg" ||  $ext[1] == "png" || $ext[1] == "gif" || $ext[1] == "JPG")
				{
					$path_file = str_replace("./","",$path);
				...
				$fileWithId = \DB::table('file_infos')->where('name',$file)->first();

				if(count($fileWithId) != 0){
					$filetype = explode('/',$fileWithId->mime_type)[1];

					$ext = explode('.',$file);
					if($ext[1] == "jpg" || $ext[1] == "jpeg" ||  $ext[1] == "png" || $ext[1] == "gif" || $ext[1] == "JPG"){
						$path_file = str_replace("./","",$path);

						return '<p><a href="'.url( $path_file . $fileWithId->id . '.' . $filetype).'" target="_blank" class="previewImage">
						<img src="'.asset( $path_file . $fileWithId->id . '.' . $filetype ).'" border="0" width="'. $width .'" class="img-square" /></a></p>';
					}else{
						$path_file = str_replace("./","",$path);
						return '<p> <a href="'.url($path_file . $file).'" target="_blank"> '.$file.' </a>';
					}
